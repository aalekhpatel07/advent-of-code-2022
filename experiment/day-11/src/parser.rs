use std::{ops::{Add, Sub, Mul, Div}, str::FromStr};
use nom::{
    IResult,
    bytes::complete::*,
...
impl BinaryOperation {
    pub fn operate<T: Add<T, Output=T> + Sub<T, Output=T> + Mul<T, Output=T> + Div<T, Output=T>>(&self) -> impl Fn(T, T) -> T {
        match self {
...
fn parse_operand(s: &str) -> IResult<&str, Operand> {
    alt((
        ...
        map(
            digit1, |digit_str: &str| Operand::Constant(digit_str.parse::<usize>().unwrap())
        ),
    ))(s)
}
...
fn parse_operation(s: &str) -> IResult<&str, Operation> {
    let (rem, (op1, _, bin_op, _, op2)) = tuple((parse_operand, space1, parse_binary_op, space1, parse_operand))(s)?;
    Ok((
        rem,
        Operation {
            operand1: op1,
...
fn parse_prefix_then_number<'a, 'b, T: FromStr>(s: &'a str, prefix: &'b str) -> IResult<&'a str, T> {
    preceded(
        tag(prefix),
        map_res(
            digit1,
...
pub fn parse_monke(s: &str) -> IResult<&str, Monke> {
    ...
    s = s.trim_start();
    let (mut s, true_case_target_monkey) = parse_prefix_then_number::<usize>(s, "If true: throw to monkey ")?;
    s = s.trim_start();
    let (mut s, false_case_target_monkey) = parse_prefix_then_number::<usize>(s, "If false: throw to monkey ")?;
    s = s.trim_start();
