    ...
    pub fn play_monke(&self, monkee_idx: usize) -> Vec<(MonkeIdx, WorryLevel)> {
        ...
        if !self.is_part_two {
            for item_index in 0..current_monke.items.len() {
                let mut bored_worry_level = current_monke.operation.apply(current_monke.items[item_index]);
                bored_worry_level /= 3;
            ...
            for item_index in 0..current_monke.items.len() {
                let bored_worry_level = current_monke.operation.apply(current_monke.items[item_index]);
                let remainder = bored_worry_level % current_monke.test_divisor;
                ...
                if remainder == 0 {
                    items.push((current_monke.throw_to_monkey_if_test, bored_worry_level % self.divisor_product));
                } else {
                    items.push((current_monke.throw_to_monkey_if_not_test, bored_worry_level % self.divisor_product));
                }
            }
        }
    ...
    pub fn play_round(&mut self) {
        for current_monke_index in 0..self.monkees.len() {
            ...
            for (monke_idx, worry_level) in items_to_throw {
                self.monkees.get_mut(monke_idx).unwrap().items.push(worry_level);
            }
        }
    }
}